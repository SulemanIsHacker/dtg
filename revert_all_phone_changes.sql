-- REVERT ALL PHONE NUMBER CHANGES
-- This will restore the database to its original state before phone number implementation

-- Step 1: Drop all versions of create_simple_purchase function
...
-- Step 2: Remove phone_number column from user_auth_codes table
ALTER TABLE user_auth_codes DROP COLUMN IF EXISTS phone_number;

-- Step 3: Drop the phone number index
DROP INDEX IF EXISTS idx_user_auth_codes_phone;

-- Step 4: Restore the original create_simple_purchase function (without phone number)
