-- SIMPLE FIX: Reliable product code generation
-- This will fix the null constraint violation

-- First, ensure the code column exists and is nullable temporarily
DO $$
BEGIN
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'product_codes') THEN
        ...
        -- Make it nullable temporarily to avoid constraint issues
        ALTER TABLE product_codes ALTER COLUMN code DROP NOT NULL;
    ELSE
...
-- Create the function with reliable product code generation
...
BEGIN
    ...
    LOOP
        ...
        -- Generate product code using a more reliable method
        v_product_code := 'PC' || EXTRACT(EPOCH FROM NOW())::BIGINT || LPAD(v_counter::TEXT, 3, '0');
