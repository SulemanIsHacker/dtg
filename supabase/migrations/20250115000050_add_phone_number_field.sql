...
-- This migration removes all phone number functionality and restores original state

-- Remove phone_number column from user_auth_codes table
ALTER TABLE user_auth_codes DROP COLUMN IF EXISTS phone_number;

-- Drop the phone number index
DROP INDEX IF EXISTS idx_user_auth_codes_phone;

-- Restore the original create_simple_purchase function (without phone number)
