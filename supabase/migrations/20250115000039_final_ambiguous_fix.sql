-- Final fix for ambiguous column reference
-- This migration fixes all ambiguous column references by using proper variable names
...
-- Create product code generation function with no ambiguity
...
BEGIN
    ...
    new_code := product_prefix || '-' || random_suffix;
    
    -- Ensure uniqueness with retry logic
    WHILE EXISTS (SELECT 1 FROM product_codes WHERE product_code = new_code) AND counter < 100 LOOP
        random_suffix := LPAD(FLOOR(RANDOM() * 10000)::TEXT, 4, '0');
        new_code := product_prefix || '-' || random_suffix;
        counter := counter + 1;
    END LOOP;
    ...
    IF EXISTS (SELECT 1 FROM product_codes WHERE product_code = new_code) THEN
        new_code := product_prefix || '-' || EXTRACT(EPOCH FROM NOW())::INTEGER::TEXT;
    END IF;
...
-- Create simplified purchase function with no ambiguity
...
-- Create approval function with no ambiguity
...
BEGIN
    ...
    BEGIN
        INSERT INTO user_subscriptions (
            ...
            false,
            'active',
            COALESCE(p_admin_notes, 'Created from approved product code: ' || code_value)
...
-- Create rejection function with no ambiguity
...
-- Create delete individual product code function with no ambiguity
...
-- Create delete user product codes function with no ambiguity
