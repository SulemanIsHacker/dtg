-- Debug function to find the exact error
...
-- Create debug function that shows exactly where it fails
...
DECLARE
    ...
    v_step TEXT;
BEGIN
    v_step := 'Starting function';
    
    -- Check if user already exists
    v_step := 'Checking if user exists';
    ...
    IF v_user_auth_code_id IS NOT NULL THEN
        ...
        v_step := 'Updating returning user';
    ...
    LOOP
        ...
        v_step := 'Adding to total amount';
        
        -- Add to total amount
        v_total_amount := v_total_amount + (v_product->>'price')::DECIMAL(10,2);
        
        v_step := 'Creating product code record';
        ...
        v_step := 'Adding to product codes array';
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            ...
            'product_id', (SELECT jsonb_array_elements(p_products)->>'product_id' LIMIT 1)
        );
END;
$$;
